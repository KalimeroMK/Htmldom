...
//		$pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
     ...
     * @return void
     */
    protected function seek($selector, &$ret, bool $lowercase = false)
    {
        global $debug_object;
        if (is_object($debug_object)) {
            $debug_object->debug_log_entry(1);
     ...
     * @return array|mixed|null
     */
    function childNodes(int $idx = - 1) { return $this->children($idx); }
